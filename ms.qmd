...
Pablo Recio-Santiago^1^,$\ddagger$ , Dalton L. Leibold^1^, Ondi Crino ^1,2^, Kris Wild^1,3^, Daniel W.A. Noble^1^

^1^ Division of Ecology and Evolution, Research School of Biology, The Australian National University, Canberra, ACT 2601, Australia &NewLine;
^2^ Flinder's University, Adelaide, SA 5001, Australia &NewLine;
^3^ Melbourne University, Melbourne, VIC 3000, Australia &NewLine;
$\ddagger$ Corresponding author: pablo.reciosantiago@anu.edu.au &NewLine;
...
Both species learnt across all treatments (@fig-results B, D), but we did not find any difference between treatments or species in the rate of learning. For _L. delicata_, hormone treatment (Control - CORT = `r format_dec(mean(c(deli_ControlCold, deli_ControlHot)) - mean(c(deli_CORTCold + deli_CORTHot)), 3)`, p~mcmc~ `r format_p(pmcmc(mean(c(deli_ControlCold, deli_ControlHot)) - mean(c(deli_CORTCold, deli_CORTHot))), 3)`), incubation temperature (Hot - Cold = `r format_dec(mean(c(deli_CORTHot, deli_ControlHot)) - mean(c(deli_CORTCold, deli_ControlCold)),3)`, p~mcmc~ `r format_p(pmcmc(mean(c(deli_CORTHot, deli_ControlHot)) - mean(c(deli_CORTCold, deli_ControlCold))),3)`), or the interaction ([(Control-Hot - CORT-Hot) - (Control-Cold - CORT-Cold)] = `r format_dec((mean(deli_ControlHot) - mean(deli_CORTHot)) - (mean(deli_ControlCold) - mean(deli_CORTCold)), 3)`, p~mcmc~ `r format_p(pmcmc((mean(deli_ControlHot) - mean(deli_CORTHot)) - (mean(deli_ControlCold) - mean(deli_CORTCold))), 3)`) did not affect learning (see @fig-results A, B). We similarly found no significant effects of  hormone (Control - CORT = `r format_dec(mean(c(guich_ControlCold, guich_ControlHot)) - mean(c(guich_CORTCold, guich_CORTHot)), 3)`, p~mcmc~ `r format_p(pmcmc(mean(c(guich_ControlCold, guich_ControlHot)) - mean(c(guich_CORTCold, guich_CORTHot))), 3)`), incubation temperature (Hot - Cold = `r format_dec(mean(c(guich_CORTHot, guich_ControlHot)) - mean(c(guich_CORTCold, guich_ControlCold)),3)`, p~mcmc~ `r format_p(pmcmc(mean(c(guich_CORTHot, guich_ControlHot)) - mean(c(guich_CORTCold, guich_ControlCold))),3)`), on learning in _L. guichenoti_ (see @fig-results C, D), but we found a significant interaction ([(Control-Hot - CORT-Hot) - (Control-Cold - CORT-Cold)] = `r format_dec((mean(guich_ControlHot) - mean(guich_CORTHot)) - (mean(guich_ControlCold) - mean(guich_CORTCold)), 3)`, p~mcmc~ `r format_p(pmcmc((mean(guich_ControlHot) - mean(guich_CORTHot)) - (mean(guich_ControlCold) - mean(guich_CORTCold))), 3)`). However, when the  (Control-Hot - CORT-Hot = `r format_dec(mean(guich_ControlHot) - mean(guich_CORTHot),3)`; p~mcmc~ `r format_p(pmcmc(mean(guich_ControlHot) - mean(guich_CORTHot)),3)`) or (Control-Cold - CORT-Cold = `r format_dec(mean(guich_ControlCold) - mean(guich_CORTCold),3)`; p~mcmc~ `r format_p(pmcmc(mean(guich_ControlCold) - mean(guich_CORTCold)),3)`).  
