...
Pablo Recio-Santiago^1^,$\ddagger$ , Dalton C. Leibold^1^, Ondi L. Crino ^1,2^, Kristoffer H. Wild^1,3^, Chris Friesen^4^, Basile Mauclaire^1,5^, Amelia Y. Peardon ^1^, Daniel W.A. Noble^1^
...
` r summary(model_age_deli)`

Results for _L. guichenoti_:
` r summary(model_age_guich)`

In the figure, the distribution of age per treatment and species:
```{r, age} 
#| label: age
