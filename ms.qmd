...
Both species learnt across all treatments  (?@tbl-data), but we did not find any difference between treatments or species in the rate of learning. For _L. delicata_, hormone treatment (Control - CORT = `r format_dec(mean((deli_ControlCold + deli_ControlHot) - (deli_CORTCold + deli_CORTHot)), 3)`, p~mcmc~ = `r format_p(pmcmc((deli_ControlCold + deli_ControlHot) - (deli_CORTCold + deli_CORTHot)), 3)`), incubation temperature (Hot - Cold = `r format_dec(mean((deli_CORTHot + deli_ControlHot) - (deli_CORTCold + deli_ControlCold)),3)`, p~mcmc~ = `r format_p(pmcmc((deli_CORTHot + deli_ControlHot) - (deli_CORTCold + deli_ControlCold)),3)`), or the interaction ([(Control-Hot - CORT-Hot) - (Control-Cold - CORT-Cold)] = `r format_dec(mean((deli_ControlHot - deli_CORTHot)-(deli_ControlCold - deli_CORTCold)), 3)`, p~mcmc~ = `r format_p(pmcmc((deli_ControlHot - deli_CORTHot)-(deli_ControlCold - deli_CORTCold)), 3)`) did not affect learning (see @fig-deli). We similarly found no significant effects of  hormone (Control - CORT = `r format_dec(mean((guich_ControlCold + guich_ControlHot) - (guich_CORTCold + guich_CORTHot)), 3)`, p~mcmc~ = `r format_p(pmcmc((guich_ControlCold + guich_ControlHot) - (guich_CORTCold + guich_CORTHot)), 3)`), incubation temperature (Hot - Cold = `r format_dec(mean((guich_CORTHot + guich_ControlHot) - (guich_CORTCold + guich_ControlCold)),3)`, p~mcmc~ = `r format_p(pmcmc((guich_CORTHot + guich_ControlHot) - (guich_CORTCold + guich_ControlCold)),3)`), or the interaction ([(Control-Hot - CORT-Hot) - (Control-Cold - CORT-Cold)] = `r format_dec(mean((guich_ControlHot - guich_CORTHot) - (guich_ControlCold - guich_CORTCold)), 3)`, p~mcmc~ = `r format_p(pmcmc((guich_ControlHot - guich_CORTHot) - (guich_ControlCold - guich_CORTCold)), 3)`) on learning in _L. guichenoti_ (see @fig-guich).  
...
Finally, when all individuals from all the treatments were pooled together and species were compared, there were not significant differences in the estimated slopes (Mean learning slope _L. delicata_ = `r format_dec(mean(deli_CORTCold + deli_ControlCold + deli_CORTHot + deli_ControlHot), 3)`, 95% CI = [`r format_dec(quantile(mean(deli_CORTCold + deli_ControlCold + deli_CORTHot + deli_ControlHot), 0.025), 3)`, `r format_dec(quantile(mean(deli_CORTCold + deli_ControlCold + deli_CORTHot + deli_ControlHot), 0.975), 3)`]; Mean learning slope _L. guichenoti_ = `r format_dec(mean(guich_CORTCold + guich_ControlCold + guich_CORTHot + guich_ControlHot), 3)`, 95% CI = [`r format_dec(quantile(mean(guich_CORTCold + guich_ControlCold + guich_CORTHot + guich_ControlHot), 0.025), 3)`, `r format_dec(quantile(mean(guich_CORTCold + guich_ControlCold + guich_CORTHot + guich_ControlHot), 0.975), 3)`]; _L. delicata_ - _L. guichenoti_ = `r format_dec(mean((deli_CORTCold + deli_ControlCold + deli_CORTHot + deli_ControlHot) - (guich_CORTCold + guich_ControlCold + guich_CORTHot + guich_ControlHot)), 3)`, p~mcmc~ = `r format_p(pmcmc((deli_CORTCold + deli_ControlCold + deli_CORTHot + deli_ControlHot) - (guich_CORTCold + guich_ControlCold + guich_CORTHot + guich_ControlHot)), 3)`).  
