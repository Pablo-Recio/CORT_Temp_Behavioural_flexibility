...
#
#| label: fig-Methods
#| fig.cap: "Experimental design of early environment manipulation and learning tasks. Stages 1-3 indicate the different phases of the habituation phase. In the associative and reversal tasks, white lids show the ramps where the food reward was not accessible."
...
#
#| label: tbl-data
#| tbl-cap: "Estimates of Reversal learning slope for all the different treatments per each task, species, and group. Mean shows the arithmetic means of the estimates obtained from the posteriors of the model, and 95% CI indicates the 95% confidence interval of the mean. All p-values were obtained using pmcmc and tested the hypothesis that the mean equals zero. In bold, those values that are significant (p-value <0.05)"
source(here("R", "func.R"))
#
...
pvalue <- format_p(sapply(estimates, pmcmc), 4)
#
# Building the df
table_df <- data.frame(
...
pvalue_2 <- format_p(sapply(estimates_2, pmcmc), 4)
#
# Building the df
